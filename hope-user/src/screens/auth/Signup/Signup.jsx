 ...
 * @description
 * A comprehensive registration screen for the Yoga Guide Management platform.
 * * Key Features:
 * - **Dynamic Profile Media:** Integrates `react-native-image-crop-picker` for high-resolution avatar selection via camera or gallery.
 * - **Multi-part Form Data:** Manages complex state for text inputs and binary image data, packaged into `FormData` for backend processing.
 * - **UX & Feedback:** Utilizes `useRef` for programmatic "shake" animations on form rejection and `react-native-animatable` for staggered entry transitions.
 * - **Asynchronous Operations:** Handles user registration via Redux `registerUser` thunk with specialized error handling for network and validation failures.
 * - **Adaptive UI:** Implements `KeyboardAvoidingView` and `ScrollView` to ensure accessibility across various device aspect ratios.
 */
